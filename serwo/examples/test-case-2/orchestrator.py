...
def orchestrator_function(context: df.DurableOrchestrationContext):
    ...
    # user dag execution
    idbl = yield context.call_activity("TaskA", serwoObject)
    rndn = []
    ayge = context.call_activity("TaskB", idbl)
    jeem = context.call_activity("TaskC", idbl)
    rndn.append(ayge)
    rndn.append(jeem)
    aifz = yield context.task_all(rndn)
    scav = yield context.call_activity("FinalTask", aifz)
    scav = insert_end_stats_in_metadata(scav)
    qvcr = yield context.call_activity("CollectLogs", scav)
    return qvcr
