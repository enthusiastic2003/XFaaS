...
def orchestrator_function(context: df.DurableOrchestrationContext):
    ...
    # user dag execution
    xdqj = yield context.call_activity("node1", serwoObject)
    eped = yield context.call_activity("node2", xdqj)
    roky = yield context.call_activity("node3", eped)
    ymyz = yield context.call_activity("node4", roky)
    ibyx = yield context.call_activity("node5", ymyz)
    uykz = yield context.call_activity("finalNode", ibyx)
    uykz = insert_end_stats_in_metadata(uykz)
    kchr = yield context.call_activity("CollectLogs", uykz)
    return kchr
