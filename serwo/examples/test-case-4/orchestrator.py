...
def orchestrator_function(context: df.DurableOrchestrationContext):
    ...
    # user dag execution
    ltdu = yield context.call_activity("node1", serwoObject)
    ctcb = []
    kzyi = context.call_activity("node2", ltdu)
    yfeq = context.call_activity("node3", ltdu)
    nace = context.call_activity("node4", ltdu)
    bzut = context.call_activity("node5", ltdu)
    ctcb.append(kzyi)
    ctcb.append(yfeq)
    ctcb.append(nace)
    ctcb.append(bzut)
    oguc = yield context.task_all(ctcb)
    laih = yield context.call_activity("finalNode", oguc)
    laih = insert_end_stats_in_metadata(laih)
    hbvy = yield context.call_activity("CollectLogs", laih)
    return hbvy
