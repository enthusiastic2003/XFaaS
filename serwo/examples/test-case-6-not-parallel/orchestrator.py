...
def orchestrator_function(context: df.DurableOrchestrationContext):
    ...
    # user dag execution
    wkgx = yield context.call_activity("node1", serwoObject)
    qbmt = []
    agtc = context.call_activity("node2", wkgx)
    lwma = context.call_activity("node3", wkgx)
    zyzv = context.call_activity("node4", wkgx)
    ijtc = context.call_activity("node5", wkgx)
    hmkv = context.call_activity("node6", wkgx)
    ncde = context.call_activity("node7", wkgx)
    qbmt.append(agtc)
    qbmt.append(lwma)
    qbmt.append(zyzv)
    qbmt.append(ijtc)
    qbmt.append(hmkv)
    qbmt.append(ncde)
    glwa = yield context.task_all(qbmt)
    vijq = yield context.call_activity("finalNode", glwa)
    vijq = insert_end_stats_in_metadata(vijq)
    lyfg = yield context.call_activity("CollectLogs", vijq)
    return lyfg
