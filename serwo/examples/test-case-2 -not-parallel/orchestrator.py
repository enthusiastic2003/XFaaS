...
def orchestrator_function(context: df.DurableOrchestrationContext):
    ...
    # user dag execution
    zfuz = yield context.call_activity("TaskA", serwoObject)
    goto = []
    tegc = context.call_activity("TaskB", zfuz)
    islh = context.call_activity("TaskC", zfuz)
    goto.append(tegc)
    goto.append(islh)
    kgfw = yield context.task_all(goto)
    qelo = yield context.call_activity("FinalTask", kgfw)
    qelo = insert_end_stats_in_metadata(qelo)
    bieg = yield context.call_activity("CollectLogs", qelo)
    return bieg
