...
def function(serwoObject ) -> SerWOObject:
    try:
        ...
        places_prompt = (
            ...
            f"Suggest exactly 5 must-visit place in {place}. "
            f"The JSON must have one key: 'places', and the value must be a list containing a 5 strings. "
            f"Example: {{\"places\": [\"Red Fort\", \"Taj Mahal\", \"Sundarban\", \"Victoria Memorial\", \"Aligarh Zoo\"]}}. Do not include explanations, formatting, or any additional text."
        )


        logging.info(f"[Step 1] Prompting Groq for place suggestions for: {places_prompt}")

        response = client.chat.completions.create(
            model="llama3-8b-8192",
            messages=[{"role": "YOU ARE AN API RESPONSE SYSTEM THAT RESPONDS IN THE FORMAT OF JSON ONLY. FOLLOW THE PROMPT TO CHECK HOW TO RESPOND", "content": places_prompt}]
        )
