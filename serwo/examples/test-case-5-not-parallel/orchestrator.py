...
def orchestrator_function(context: df.DurableOrchestrationContext):
    ...
    # user dag execution
    mmsy = yield context.call_activity("node1", serwoObject)
    efzw = yield context.call_activity("node2", mmsy)
    equc = yield context.call_activity("node3", efzw)
    udbo = yield context.call_activity("node4", equc)
    xfkh = yield context.call_activity("node5", udbo)
    imyk = yield context.call_activity("node6", xfkh)
    puch = yield context.call_activity("finalNode", imyk)
    puch = insert_end_stats_in_metadata(puch)
    uhor = yield context.call_activity("CollectLogs", puch)
    return uhor
