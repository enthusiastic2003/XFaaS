...
def orchestrator_function(context: df.DurableOrchestrationContext):
    ...
    # user dag execution
    yoic = yield context.call_activity("node1", serwoObject)
    ofmf = []
    hxay = context.call_activity("node2", yoic)
    gmbt = context.call_activity("node3", yoic)
    cnuk = context.call_activity("node4", yoic)
    ottb = context.call_activity("node5", yoic)
    xyid = context.call_activity("node6", yoic)
    yxuv = context.call_activity("node7", yoic)
    ofmf.append(hxay)
    ofmf.append(gmbt)
    ofmf.append(cnuk)
    ofmf.append(ottb)
    ofmf.append(xyid)
    ofmf.append(yxuv)
    hgsh = yield context.task_all(ofmf)
    zojs = yield context.call_activity("finalNode", hgsh)
    zojs = insert_end_stats_in_metadata(zojs)
    qyso = yield context.call_activity("CollectLogs", zojs)
    return qyso
