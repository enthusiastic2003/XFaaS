...
def orchestrator_function(context: df.DurableOrchestrationContext):
    ...
    # user dag execution
    jpjg = yield context.call_activity("node1", serwoObject)
    vsan = yield context.call_activity("node2", jpjg)
    jwih = yield context.call_activity("node3", vsan)
    lrik = yield context.call_activity("node4", jwih)
    zqvv = yield context.call_activity("finalNode", lrik)
    zqvv = insert_end_stats_in_metadata(zqvv)
    clwt = yield context.call_activity("CollectLogs", zqvv)
    return clwt
