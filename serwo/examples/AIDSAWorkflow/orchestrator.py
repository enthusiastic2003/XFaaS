...
def orchestrator_function(context: df.DurableOrchestrationContext):
    ...
    # user dag execution
    nkcr = yield context.call_activity("node1", serwoObject)
    uhve = yield context.call_activity("ProcessAllSubproblems", nkcr)
    jlsd = yield context.call_activity("AssembleFinalCode", uhve)
    wbsl = yield context.call_activity("FinalizeOutput", jlsd)
    wbsl = insert_end_stats_in_metadata(wbsl)
    mmgl = yield context.call_activity("CollectLogs", wbsl)
    return mmgl
