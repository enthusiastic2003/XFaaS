...
def orchestrator_function(context: df.DurableOrchestrationContext):
    ...
    # user dag execution
    bfxc = yield context.call_activity("node1", serwoObject)
    ejlq = []
    aclg = context.call_activity("node2", bfxc)
    kzom = context.call_activity("node3", bfxc)
    rfqo = context.call_activity("node4", bfxc)
    ejlq.append(aclg)
    ejlq.append(kzom)
    ejlq.append(rfqo)
    bbpi = yield context.task_all(ejlq)
    uetp = yield context.call_activity("finalNode", bbpi)
    uetp = insert_end_stats_in_metadata(uetp)
    upty = yield context.call_activity("CollectLogs", uetp)
    return upty
