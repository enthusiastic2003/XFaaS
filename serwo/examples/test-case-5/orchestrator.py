...
def orchestrator_function(context: df.DurableOrchestrationContext):
    ...
    # user dag execution
    yslv = yield context.call_activity("node1", serwoObject)
    fqig = []
    shqi = context.call_activity("node2", yslv)
    ncse = context.call_activity("node3", yslv)
    qppf = context.call_activity("node4", yslv)
    fbnl = context.call_activity("node5", yslv)
    fhxk = context.call_activity("node6", yslv)
    fqig.append(shqi)
    fqig.append(ncse)
    fqig.append(qppf)
    fqig.append(fbnl)
    fqig.append(fhxk)
    pvmb = yield context.task_all(fqig)
    wsyz = yield context.call_activity("finalNode", pvmb)
    wsyz = insert_end_stats_in_metadata(wsyz)
    bjad = yield context.call_activity("CollectLogs", wsyz)
    return bjad
